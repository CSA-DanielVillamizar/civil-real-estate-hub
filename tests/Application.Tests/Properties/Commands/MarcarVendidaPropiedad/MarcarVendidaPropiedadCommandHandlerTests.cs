...
using Plataforma.Application.Properties.Commands.MarcarVendidaPropiedad;
...
namespace Plataforma.Application.Tests.Properties.Commands.MarcarVendidaPropiedad;

public sealed class MarcarVendidaPropiedadCommandHandlerTests
{
    private readonly Mock<IPropertyRepository> _propertyRepositoryMock = new();
    private readonly MarcarVendidaPropiedadCommandHandler _sut;

    public MarcarVendidaPropiedadCommandHandlerTests()
    {
        _sut = new MarcarVendidaPropiedadCommandHandler(_propertyRepositoryMock.Object);
    }
    ...
    [Fact]
    public async Task Handle_ConPropiedadPublicada_LaMarcaVendidaYPersiste()
    {
        ...
        var resultado = await _sut.Handle(new MarcarVendidaPropiedadCommand(propiedad.Id.Value), CancellationToken.None);

        resultado!.Estado.Should().Be(nameof(EstadoPropiedad.Vendida));
        _propertyRepositoryMock.Verify(r => r.UpdateAsync(propiedad, It.IsAny<CancellationToken>()), Times.Once);
    }

    [Fact]
    public async Task Handle_ConPropiedadYaVendida_LanzaPropiedadEnEstadoInvalidoException()
    {
        var propiedad = CrearPropiedadPublicada();
        propiedad.MarcarVendida();
        _propertyRepositoryMock.Setup(r => r.GetByIdAsync(propiedad.Id, It.IsAny<CancellationToken>())).ReturnsAsync(propiedad);

        var accion = () => _sut.Handle(new MarcarVendidaPropiedadCommand(propiedad.Id.Value), CancellationToken.None);
    ...
    public async Task Handle_ConPropiedadInexistente_DevuelveNull()
    {
        ...
        var resultado = await _sut.Handle(new MarcarVendidaPropiedadCommand(id), CancellationToken.None);

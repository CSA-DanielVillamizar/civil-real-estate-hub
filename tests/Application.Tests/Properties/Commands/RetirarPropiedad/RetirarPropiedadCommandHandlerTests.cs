...
using Plataforma.Application.Properties.Commands.RetirarPropiedad;
...
namespace Plataforma.Application.Tests.Properties.Commands.RetirarPropiedad;

public sealed class RetirarPropiedadCommandHandlerTests
{
    private readonly Mock<IPropertyRepository> _propertyRepositoryMock = new();
    private readonly RetirarPropiedadCommandHandler _sut;

    public RetirarPropiedadCommandHandlerTests()
    {
        _sut = new RetirarPropiedadCommandHandler(_propertyRepositoryMock.Object);
    }
    ...
    [Fact]
    public async Task Handle_ConPropiedadPublicada_LaRetiraYPersiste()
    {
        ...
        var resultado = await _sut.Handle(new RetirarPropiedadCommand(propiedad.Id.Value), CancellationToken.None);

        resultado!.Estado.Should().Be(nameof(EstadoPropiedad.Retirada));
    ...
    public async Task Handle_ConPropiedadVendida_LanzaPropiedadEnEstadoInvalidoException()
    {
        ...
        var accion = () => _sut.Handle(new RetirarPropiedadCommand(propiedad.Id.Value), CancellationToken.None);
    ...
    public async Task Handle_ConPropiedadInexistente_DevuelveNull()
    {
        ...
        var resultado = await _sut.Handle(new RetirarPropiedadCommand(id), CancellationToken.None);
